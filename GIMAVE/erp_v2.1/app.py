...
#Upload e visualização dos cadastros de credenciados
        ...
        try:
            ...
            if not df.empty:
                try:
                    ...
                    # Buscar pares codigo + produto já existentes no banco
                    cursor.execute("SELECT codigo, produto FROM credenciados_cpl")
                    existentes = {(row["codigo"], row["produto"]) for row in cursor.fetchall()}

                    dados_validos = []

                    for _, row in df.iterrows():

                        codigo_val = None if pd.isna(row["codigo"]) else int(row["codigo"])
                        produto_val = None if pd.isna(row["produto"]) else row["produto"]

                        #Se já existir o par (codigo, produto), pula
                        if (codigo_val, produto_val) in existentes:
                            continue 
                        ...
                        dados_validos.append((
                            ...
                            None if pd.isna(row["taxa_servico"]) else row["taxa_servico"],
                            None if pd.isna(row["taxa_interconexao"]) else row["taxa_interconexao"],
                    ...
                    if dados_validos:
                        query = """
                            INSERT INTO credenciados_cpl (
                                ...
                                taxa_servico, taxa_interconexao, idpj_produto, data_cadastro, data_utransacao
                            )
                            VALUES (
                                %s, %s, %s, %s, %s, %s,
                                %s, %s, %s, %s, %s, %s,
...
def api_credenciadosCPL():
    ...
    # Referência do IPCA enviada pelo frontend ("ANO-MES")
    ipca_ref = request.form.get("ipca_ref")
    ...
    query = f"""
        ...
        LIMIT %s OFFSET %s
    """
    params_query = params + [length, start]
    ...
    if ipca_ref:
        try:
            ano, mes = ipca_ref.split("-")
            cursor.execute("""
                SELECT pct FROM indexadores
                WHERE nome='IPCA' AND ano=%s AND mes=%s
            """, (ano, mes))
            idx = cursor.fetchone()
            if idx:
                pct_ipca = float(idx["pct"])
        except:
            pct_ipca = 0.0
    ...
    for r in rows:
        ...
        # tax originais
        ts = float(r["taxa_servico"]) if r["taxa_servico"] else 0.0
        ti = float(r["taxa_interconexao"]) if r["taxa_interconexao"] else 0.0

        # taxas reajustadas
        ts_n = ts * (1 + pct_ipca / 100)
        ti_n = ti * (1 + pct_ipca / 100)

        cred[cod]["produtos"].add(r["produto"])
        cred[cod]["taxa_servico"].append(ts)
        cred[cod]["taxa_servico_n"].append(ts_n)
        cred[cod]["taxa_interconexao"].append(ti)
        cred[cod]["taxa_interconexao_n"].append(ti_n)
    ...
    for c in cred.values():
        ...
        if dc:
            try:
                if isinstance(dc, str):
                    dc = datetime.strptime(dc, "%Y-%m-%d %H:%M:%S")
        ...
        if du:
            try:
                if isinstance(du, str):
                    du = datetime.strptime(du, "%Y-%m-%d %H:%M:%S")
        ...
        ts = c["taxa_servico"][0]
        ts_n = c["taxa_servico_n"][0]
        ti = c["taxa_interconexao"][0]
        ti_n = c["taxa_interconexao_n"][0]
        ...
        data.append([
            ...
            c["uf"],
            produtos_concat,
            f"<span class='{badge_serv}'>{ts:.2f}%</span>",
            f"<span class>{ts_n:.2f}%</span>",
            f"<span class='{badge_inter}'>{ti:.2f}%</span>",
            f"<span class>{ti_n:.2f}%</span>",
            data_cadastro,
            data_utransacao,
        ])
...
#Update e consulta de taxas divergentes de um mesmo código
@modulo_requerido('COMPLIANCE')
@app.route("/validartaxasCPL", methods=["GET"])
def validartaxasCPL():
    if 'username' not in session:
    ...
    cursor.execute("SELECT * FROM credenciados_cpl ORDER BY codigo")
    rows = cursor.fetchall()
    ...
    # Monta lista final com divergências
    divergencias = []
    for c in cred.values():
        serv_diff = len(set(c["taxa_servico"])) > 1
        inter_diff = len(set(c["taxa_interconexao"])) > 1

        if serv_diff or inter_diff:
            divergencias.append({
                "codigo": c["codigo"],
                ...
                "cidade": c["cidade"],
                "uf": c["uf"],
                "produtos": "/".join(sorted(list(c["produtos"]))),
                "taxa_servico": list(set(c["taxa_servico"])),
                "taxa_interconexao": list(set(c["taxa_interconexao"])),
            })

    return render_template("validartaxasCPL.html", divergencias=divergencias)

@modulo_requerido('COMPLIANCE')
@app.route("/validartaxasCPL/update", methods=["POST"])
def update_validacao_taxas_cpl():
    if 'username' not in session:
        return jsonify({"error": "unauthorized"}), 403

    codigo = request.form.get("codigo")

    nova_serv = request.form.get("taxa_servico")
    nova_inter = request.form.get("taxa_interconexao")
    ...
    if nova_serv and nova_serv.strip():
        campos.append("taxa_servico = %s")
        valores.append(nova_serv)

    if nova_inter and nova_inter.strip():
        campos.append("taxa_interconexao = %s")
        valores.append(nova_inter)

    if campos:
        sql = f"""
            UPDATE credenciados_cpl
            SET {", ".join(campos)}
            WHERE codigo = %s
        """
    ...
    cursor.execute("SELECT * FROM credenciados_cpl WHERE codigo = %s LIMIT 1", (codigo,))
    row = cursor.fetchone()

    if not row:
        flash("Credenciado não encontrado após atualização.", "erro")
        return redirect("/validartaxasCPL")
